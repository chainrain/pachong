...
2. 修改为RedisCrawlSpider的分布式爬虫
2.1 修改继承关系: 继承RedisCrawlSpider
2.2 把start_urls改为redis_key
2.3 修改配置文件(如果前面配置过了, 就可以省略了)
"""

class AmazonSpider(RedisCrawlSpider):
    name = 'RedisCrawlSpider_amazon'
    allowed_domains = ['amazon.cn']
    # start_urls = ['https://www.amazon.cn/%E5%9B%BE%E4%B9%A6/b/ref=sa_menu_top_books_l1?ie=UTF8&node=658390051']
    # redis_key: 用于指定起始URL在Redis数据库中key, 不是redis数据库的地址
